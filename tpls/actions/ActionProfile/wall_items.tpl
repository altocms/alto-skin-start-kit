{foreach $aWall as $oWall}
	...
	<div id="wall-item-{$oWall->getId()}" class="js-wall-item comment-wrapper">
		<div class="comment">
			<a href="{$oWallUser->getProfileUrl()}"><img src="{$oWallUser->getAvatarUrl(64)}" alt="{$oWallUser->getDisplayName()}" class="comment-avatar" /></a>

			<ul class="list-unstyled list-inline small comment-info">
				<li class="comment-author"><a href="{$oWallUser->getProfileUrl()}">{$oWallUser->getDisplayName()}</a></li>
