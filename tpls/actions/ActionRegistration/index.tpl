...
<form action="{router page='registration'}" method="post" id="registration-form">
	...
	{hookb run="registration_captcha"}
		<div class="form-group">
			...
			<img src="{Config::Get('path.root.engine_lib')}/external/kcaptcha/index.php?{$_sPhpSessionName}={$_sPhpSessionId}"
				onclick="this.src='{Config::Get('path.root.engine_lib')}/external/kcaptcha/index.php?{$_sPhpSessionName}={$_sPhpSessionId}&n='+Math.random();"
				class="captcha-image" />
